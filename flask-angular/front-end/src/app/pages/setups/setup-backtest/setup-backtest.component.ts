...
@Component({
  selector: 'app-setup-backtest',
  templateUrl: './setup-backtest.component.html',
  styleUrls: ['./setup-backtest.component.scss']
})
export class SetupBacktestComponent implements OnInit {
  ...
  onRun() {
    this.setupService.runBacktest(this.formBacktest.value)
      .subscribe({
        next: (resp) => {
  ...
  onRunToCSV() {
    ...
    this.setupService.runBacktestToCsv(body)
      .subscribe({
        next: (resp) => {
  ...
  onViewTrades(ativo: string) {
    this.modalService.open('modalTradesBackTest')
  ...
  onCloseTrade() {
    this.modalService.close('modalTradesBackTest')
  }
}
