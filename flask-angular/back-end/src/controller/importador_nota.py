# @author Marildo Cesar 22/04/2024
import copy
...
from ..model import db_connection, Operacao, NotaCorretagem, CompraVenda, TipoNota
...
class ImportadorNota:
    ...
    @classmethod
    def import_notas(cls, notas: List[Nota]):
        cls.__load_save_ativos(notas)
        for item in notas:
            cls.__import_nota(item)

    @classmethod
    def __load_save_ativos(cls, notas):
        ativos = set()
        for n in notas:
    ...
    def __import_nota(cls, item: Nota):
        map_order = {'C': lambda x: x.data_venda, 'V': lambda x: x.data_compra}
    ...
    @staticmethod
    def __appoint_daytrade(operacoes):
